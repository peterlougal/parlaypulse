...
function scoreProp(p: PropRow, preferLong: boolean): number {
  // Higher PF rating + streak wins; for longshot buckets bias longer odds
  let s = p.rating * 2 + p.streak * 3;
  if (preferLong) s += Math.min(p.dec, 4) * 8;
  else s += Math.max(0, 3 - p.dec) * 10; // prefer shorter prices for 10x
  return s;
}
      ...
      <main className="max-w-5xl mx-auto px-4 py-8 space-y-6">
        <section className="bg-zinc-900/60 border border-zinc-800 rounded-2xl p-5">
          ...
          <p className="text-xs text-zinc-600 mt-2">
            Live data loaded: {availableNote}. NFL/NBA/NHL/MLS unlock when you drop in weekly CSVs.
          </p>
        </section>
        ...
        {phase === "results" && (
          <section className="space-y-4">
            ...
            <p className="text-[11px] text-zinc-600 text-center pt-2">
              Built from today&apos;s PropFinder export — not a guarantee. Highest-tier feature
              preview.
            </p>
          </section>
        )}
