...
export default function MlbTrackerPage() {
  ...
  const [favMeta, setFavMeta] = useState<
    Record<string, { name: string; team: string; opponent: string; hits: number; homeRuns: number; totalBases: number; rbi: number; runs: number; stolenBases: number; strikeOuts: number }>
  >({});
  ...
  useEffect(() => {
    ...
    try {
      ...
      const m = localStorage.getItem("pp-mlb-fav-meta");
      if (f) setFavorites(new Set(JSON.parse(f)));
      if (p) setFavProps(JSON.parse(p));
      if (m) setFavMeta(JSON.parse(m));
    } catch {
      /* ignore */
    }
  }, []);
  ...
  useEffect(() => {
    if (!mounted) return;
    localStorage.setItem("pp-mlb-fav-meta", JSON.stringify(favMeta));
  }, [favMeta, mounted]);
  ...
  function toggleFav(p: PlayerRow) {
    const id = p.id;
    setFavorites((prev) => {
      ...
      if (next.has(id)) {
        ...
        next.add(id);
        if (!favProps[id]) {
          setFavProps((fp) => ({ ...fp, [id]: p.isPitcher ? "k_4.5" : "hits_0.5" }));
        }
        setFavMeta((fm) => ({
          ...fm,
          [id]: {
  ...
  // Keep meta stats fresh for favorited players
  useEffect(() => {
    if (!players.length || !favorites.size) return;
    setFavMeta((fm) => {
      const next = { ...fm };
      let changed = false;
    ...
    <div className="min-h-screen">
        ...
        <div className="max-w-5xl mx-auto px-4 py-4 flex items-center justify-between">
          <div className="flex items-center gap-6">
            ...
            <nav className="hidden md:flex gap-3 text-sm flex-wrap">
              ...
              <Link href="/mlb-tracker" className="text-emerald-400 font-medium">MLB</Link>
      ...
      <main className="max-w-5xl mx-auto px-4 py-8 space-y-6">
        <section className="bg-zinc-900/60 border border-zinc-800 rounded-2xl p-6">
          <div className="flex flex-wrap items-start justify-between gap-4">
            ...
            <div className="flex flex-col gap-2 items-end">
              ...
              <p className="text-[10px] text-zinc-600">Auto-updates every 90s</p>
            </div>
          </div>
          {error && (
        ...
        <footer className="text-center text-xs text-zinc-600 pt-4 pb-6">
          ...
          Auto-updates about every 90 seconds — no refresh button needed.
        </footer>
      </main>
    </div>
  );
